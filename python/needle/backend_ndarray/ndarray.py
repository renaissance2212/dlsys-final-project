...
class NDArray:
    ...
    def cudnn_conv_forward(data, filter,stride=1, padding=0):
        ...
        out = NDArray(np.zeros((N,(H-KH+2*padding+1)//stride,(W-KW+2*padding+1)//stride,C_out)), device=data.device)
        out = out.compact()
        data.device.CudnnConvForward(data.compact()._handle, w._handle, out._handle, N, C_in, H, W, C_out, KH, KW, stride, padding)
        return out
